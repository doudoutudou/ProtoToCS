...
::协议文件路径, 最后不要跟“\”符号
set SOURCE_FOLDER=..\Assets\Hotfix\Net\Protol

::C#编译器路径
set CS_COMPILER_PATH=.\ProtoGen\protogen.exe
::C#文件生成路径, 最后不要跟“\”符号
set CS_TARGET_PATH=..\Assets\Hotfix\Net\Protocal
...
for /f "delims=" %%i in ('dir /b "%SOURCE_FOLDER%\*.proto"') do (
	...
	%CS_COMPILER_PATH% +names=original --proto_path=%SOURCE_FOLDER% --csharp_out=%CS_TARGET_PATH% %%i
)
